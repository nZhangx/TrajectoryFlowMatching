...
class FM_baseline(torch.nn.Module):
    ...
    def forward_train(self, x):
        ...
        return torch.cat([result[:,:-1], result[:,-1].unsqueeze(1)], dim=1)
...
class MLP_FM(pl.LightningModule):
    def __init__(self, 
                 treatment_cond,
                 dim=2, 
                 w=64, 
        ...
        self.lr = lr
        self.sigma = sigma
        self.naming = "FM_baseline_"+implementation
    ...
    def test_trajectory_ode(self,pt_tensor):
        ...
        for i in range(len_path): 
            ...
            with torch.no_grad():
                ...
                if i == 0:
                    testpt = torch.cat([x0_values[i].unsqueeze(0)],dim=1)
